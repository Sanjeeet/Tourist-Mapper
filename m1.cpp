...
bool load_map(std::string map_name) {
    ...
    DataStructure::which_map = map_name;
    ...
    for (unsigned i = 0; i < getNumberOfStreetSegments(); i++) {
        ...
        temp_info = getStreetSegmentInfo(i); //gets object for given street segment
        ...
        if (num_of_curve_points != 0) { //if there is curve points 
            ...
            distance = find_distance_between_two_points(intersection_1_position, getStreetSegmentCurvePoint(i, 0));
            //computes distance between the first first curve point and the last curve point, and adds it to total distance 
            for (unsigned j = 0; j < num_of_curve_points - 1; j++) {
                distance += find_distance_between_two_points(getStreetSegmentCurvePoint(i, j), getStreetSegmentCurvePoint(i, j + 1));
            }
            //computes the distance from the last curve point to the intersection "TO", and adds it
            distance += find_distance_between_two_points(getStreetSegmentCurvePoint(i, num_of_curve_points - 1), intersection_2_position);
        } else //if it is straight street just finds distance between two points
            distance = find_distance_between_two_points(intersection_1_position, intersection_2_position);
         
        DataStructure::street_seg_to_street_segment_length.push_back(distance);
    }
...
//        DataStructure::vector_of_all_edges
    }
...
//
//    return (distance);
    return (DataStructure::street_seg_to_street_segment_length[street_segment_id]);
}
...
double find_street_segment_travel_time(unsigned street_segment_id) {
    ...
    StreetSegmentInfo temp_info; // temporary object for each street segment 
    temp_info = DataStructure::street_seg_id_to_street_seg_info[street_segment_id];
