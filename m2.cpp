...
void load_struct() {
    DataStructure::courier_counter = 0;
...
void draw_map() {
    ...
    create_button("Window", "Courier Path", act_on_courier_path_button_func);
    create_button("Window", "Drop Off", act_on_drop_off_button_func);
    create_button("Window", "Pick Up", act_on_pick_up_button_func);
    create_button("Window", "Depot", act_on_depot_button_func);
...
void draw_screen() {
    ...
    draw_courier_intersections_highlight();
...
void act_on_clear_button_func(void (*drawscreen_ptr) (void)) {
    ...
    DataStructure::draw_courier_path.clear();
        DataStructure::depot_vector.clear();
    DataStructure::deliveries_vector.clear();
    DataStructure::pick_up_vector.clear();
    DataStructure::drop_off_vector.clear();
...
//
//    for (auto i = DataStructure::test.begin(); i != DataStructure::test.end(); i++) {
//
//        info = getStreetSegmentInfo(*i);
//        start = info.from;
//        finish = info.to;
...
//            if (num_of_curve_points != 0) { //if there is curve points 
//
//                draw_shortest_edges_with_curve_point2(temp_start, getStreetSegmentCurvePoint(*i, 0), info);
//
//                for (unsigned k = 0; k < num_of_curve_points - 1; k++) {
//                    draw_shortest_edges_with_curve_point2(getStreetSegmentCurvePoint(*i, k), getStreetSegmentCurvePoint(*i, k + 1), info);
//                }
//
//                draw_shortest_edges_with_curve_point2(getStreetSegmentCurvePoint(*i, num_of_curve_points - 1), temp_finish, info);
//            } else
//                draw_shortest_edges_with_curve_point2(temp_start, temp_finish, info);
//        }
//    }
...
void display_directions() {
    ...
    for (unsigned i = 0; i < street_segs_path.size(); i++) {
        ...
        if (counter > 0 && (i > 0 && current_street_id != previous_street_id || street_segs_path[i] == street_segs_path.back())) {
            //Set up points to calculate angle
            LatLon temp1 = getIntersectionPosition(starting_each_street);
            LatLon temp2 = getIntersectionPosition(temp_info.from);
            starting_each_street = end_previous_street;
            end_previous_street = temp_info.from;
            LatLon temp3 = getIntersectionPosition(starting_each_street);
        ...
        else if (counter == 0 && (i > 0 && (current_street_id != previous_street_id || (current_street_id == previous_street_id && street_segs_path[i] == street_segs_path.back())))) {
            counter++; //will only enter this if statement once because of this variable
...
void act_on_depot_button_func(void (*drawscreen_ptr) (void)) {

       
        unsigned id1 = DataStructure::intersection_ids[DataStructure::courier_counter];
        update_message("Depot Registered.");

        DataStructure::depot_vector.push_back(id1);

        DataStructure::courier_counter++;
        draw_screen();
    
}


void act_on_pick_up_button_func(void (*drawscreen_ptr) (void)) {

        DataStructure::id_of_pick_up = DataStructure::intersection_ids[DataStructure::courier_counter];
        update_message("Pick Up Registered.");
     
        DataStructure::pick_up_vector.push_back(DataStructure::id_of_pick_up);
...
void act_on_drop_off_button_func(void (*drawscreen_ptr) (void)) {


        unsigned id1 = DataStructure::intersection_ids[DataStructure::courier_counter];
        update_message("Drop Off Registered.");

        DataStructure::drop_off_vector.push_back(id1);
        
        DataStructure::deliveries_vector.push_back(DeliveryInfo(DataStructure::id_of_pick_up, id1)); 
...
void act_on_courier_path_button_func(void (*drawscreen_ptr) (void)) {
    DataStructure::courier_counter = 0;

    
    DataStructure::draw_courier_path = traveling_courier(DataStructure::deliveries_vector,DataStructure::depot_vector );
...
void draw_courier() {
    ...
    for (auto i = DataStructure::draw_courier_path.begin(); i != DataStructure::draw_courier_path.end(); i++) {
...
void draw_courier_intersections_highlight() {

    for (unsigned k = 0; k < DataStructure::depot_vector.size(); k++) {

        LatLon temp1 = getIntersectionPosition(DataStructure::depot_vector[k]);
        float temp1_lat = temp1.lat * DEG_TO_RAD;
        float temp1_lon = temp1.lon * DEG_TO_RAD;
        float lat_average = (maxLat + minLat) / 2;
        float x_start = temp1_lon * cos(lat_average);
        ...
        for (unsigned k = 0; k < DataStructure::drop_off_vector.size(); k++) {

        LatLon temp1 = getIntersectionPosition(DataStructure::drop_off_vector[k]);
        float temp1_lat = temp1.lat * DEG_TO_RAD;
        float temp1_lon = temp1.lon * DEG_TO_RAD;
        float lat_average = (maxLat + minLat) / 2;
        float x_start = temp1_lon * cos(lat_average);
    ...
    for (unsigned k = 0; k < DataStructure::pick_up_vector.size(); k++) {

        LatLon temp1 = getIntersectionPosition(DataStructure::pick_up_vector[k]);
        float temp1_lat = temp1.lat * DEG_TO_RAD;
        float temp1_lon = temp1.lon * DEG_TO_RAD;
        float lat_average = (maxLat + minLat) / 2;
        float x_start = temp1_lon * cos(lat_average);
